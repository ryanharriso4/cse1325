...
public class ThreadsOfSuduko {
    public static void main(String[] args) {
        ...
        try {
            ...
            HashSet<Thread> threads = new <Thread>HashSet();
            int range = (int) Math.ceil((double) (suds.length - 1) / numThreads);
            int numSuds = suds.length - 1;
            for (int i = 0; i < numThreads; i++) {
                final int max = numSuds;
                final int min;
                final int count = i;
                if ((numSuds - range) < 0)
                    min = 0;
                    ...
                    min = numSuds - range;
                threads.add(new Thread(() -> solveSuds(min, max, count)));
